...
from ..retrieval.bm25 import BM25Index, bm25_topk
...
class ExternalDataStore:
    ...
    def __init__(
        ...
        # ::: Code Generated by Copilot 3a1e7b02-2fb1-4b8c-9c07-4ed9c5f0a112. This comment will be removed automatically after the file is saved :::
        # Cached BM25 index over the full ``_docs`` corpus. Built once at
        # ``load_all()`` time; every ``search()`` reuses it instead of
        # re-tokenizing and re-constructing ``BM25Okapi`` (which is O(N))
        # on every query. Invalidated when ``_docs`` is repopulated.
    ...
    def load_all(self) -> None:
        ...
        # ::: Code Generated by Copilot 3a1e7b02-2fb1-4b8c-9c07-4ed9c5f0a112. This comment will be removed automatically after the file is saved :::
        # Build the BM25 index once, up front. This is O(N * avg_len) in
        # tokenization but only runs at load time; subsequent searches are
        # O(N * avg_query_len) instead of O(N * avg_doc_len) each.
        if self._docs:
            try:
                self._bm25_index = BM25Index([d[0] for d in self._docs])
                log.info("BM25 index built over %d chunks", len(self._docs))
            except Exception as e:
                log.warning("Failed to build BM25 index: %s", e)
    ...
    def search(
        ...
        *,
        ...
        # ::: Code Generated by Copilot 3a1e7b02-2fb1-4b8c-9c07-4ed9c5f0a112. This comment will be removed automatically after the file is saved :::
        # BM25 pre-filter. Fast path: when no per-query filters are applied
        # (candidates == full corpus) we reuse the pre-built index.
        # Otherwise fall back to the ad-hoc ``bm25_topk`` over the filtered
        # subset — still correct, just slower.
        texts = [d[0] for d in candidates]
        if (
            self._bm25_index is not None
            and len(candidates) == len(self._docs)
        ):
            top_idx = self._bm25_index.topk(query, k=min(bm25_top, len(texts)))
        else:
