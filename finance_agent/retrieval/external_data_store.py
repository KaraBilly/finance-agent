...
class ExternalDataStore:
    ...
    # ::: Code Generated by Copilot 8f4b2c1e-9d3a-4e5f-a1b2-c3d4e5f60002. This comment will be removed automatically after the file is saved :::
    @staticmethod
    def _build_evidence_title(file_path: str, meta: dict[str, Any], source_kind: str) -> str:
        """Produce a distinguishing Evidence title for a retrieved chunk.

        Preference order:
        1. Filename stem (e.g. ``比亚迪股份有限公司 2025年年报（2026-03-27）``)
           — this is what the user actually recognises for filings/PDFs.
        2. ``<source_kind>: <symbol>`` fallback.

        A ``§N/M`` chunk-position suffix is appended when the chunker
        exposed ``chunk_index`` / ``total_chunks``, so multiple passages
        from the same document don't render as identical bullets in the
        synthesizer's ``## Evidence`` section.
        """
        stem = ""
        if file_path:
            try:
        ...
        base = stem or f"{source_kind.upper()}: {symbol}"

        chunk_index = meta.get("chunk_index")
        total_chunks = meta.get("total_chunks")
        if isinstance(chunk_index, int) and isinstance(total_chunks, int) and total_chunks > 1:
            # 1-based for humans.
            return f"{base} §{chunk_index + 1}/{total_chunks}"
        return base
    ...
    def search(
        ...
        *,
        ...
        for text, meta, source_kind in pre:
            ...
            title = self._build_evidence_title(file_path, meta, source_kind)
            ev = Evidence(
                text=text,
