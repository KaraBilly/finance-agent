...
- 领域: A 股个人交易辅助 (为什么只做 A 股: 中文语料对 doubao 更友好, 数据源可直接对接**东财 / 巨潮官方 JSON**免费获取, 时间受限时聚焦一个市场比"看起来什么都能做"更能讲清楚取舍)。
...
│  EastmoneyMarketProvider              (市场数据, 东财官方 JSON)  │
│  EastmoneyFinancialsProvider          (财务报表, 东财官方 JSON)  │
│  CninfoApiFilingsProvider             (公告/年报, 巨潮官方 JSON) │
...
┌─────────────────────────────────────────────────────────────────┐
│                      External APIs                              │
│  Volcengine Ark │ DeepSeek API │ eastmoney │ cninfo │ Tavily │ SQLite │
└─────────────────────────────────────────────────────────────────┘
```
...
- 直接调东财和巨潮官方 JSON (它们自己的网站前端就用) 更稳、更快, 且完全没有 token / 授权门槛。详见 `DATA_SOURCES.md`。
